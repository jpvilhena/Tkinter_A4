...
def create_client(db: Session, client: schemas.ClientCreate):
    db_obj = models.Cliente(**client.dict())
    db.add(db_obj)
    db.commit()
    db.refresh(db_obj)
    return db_obj
...
def create_truck(db: Session, truck: schemas.TruckCreate):
    db_obj = models.Caminhao(**truck.dict())
    db.add(db_obj)
    db.commit()
    db.refresh(db_obj)
    return db_obj
...
def get_truck(db: Session, truck_id: int):
    return db.query(models.Caminhao).filter(models.Caminhao.id_caminhao == truck_id).first()


def list_trucks(db: Session, skip: int = 0, limit: int = 100):
    return db.query(models.Caminhao).offset(skip).limit(limit).all()
